...
#nodes$year <- str_extract(nodes$id, )
#unique(c(edgelist$from, edgelist$to))
...
write.csv(edges, "my_edgelist.csv", row.names = F)
write.csv(nodes, "my_nodes.csv", row.names = F)
...
# List of studyIDs
studies <- as.data.frame(c(1:200))
colnames(studies) <- c("studyID")
...
if (study == 1){ #add first study to chain (no citations)
  ...
  #create citations by looping through each of the previous studies
    ...
    for (citedID in unique(chain$studyID)){
      ...
      #binary choice of whether it cites it or not
      is_cited <- rbinom(1, 1, 0.5)
