do_analyses_1 <- function() {
  ...
  if (do_pp_citation == TRUE) {
    ...
    for (experiment in 1:n_experiments_per_repeat) {
      # for every experiment get the relevant data set
      this_data_set <- data_sets[data_sets$data_set == experiment,]
      this_empty_df <- as.data.frame(matrix(0,nrow = 1, ncol = 4))  
      colnames(this_empty_df) <- colnames(empty_df)
      ...
      this_empty_df$pp_u <- fixef(model)[,1][[4]]
      this_empty_df$pp_sig <- fixef(model)[,2][[4]] 
      this_empty_df$studyID <- this_data_set$studyID[1]
      
      # binding into df with all PP values
      
      empty_df <- rbind(empty_df, this_empty_df)
